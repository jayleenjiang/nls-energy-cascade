...
int main() {
    ...
    for (int n : n_values) {
        ...
        outfile.open("energy_no_summation" + std::to_string(n)+ ".csv");
        outfile << "mode,energy\n";
        for (int i = 0; i < n; ++i) {
...
void run_simulation(int n, std::mt19937& rng, Vector& accumulated_energy) {
    ...
    for (int i = 0; i < N_steps; ++i) {
        ...
        // remove the term inclduing total_mass_I
        ...
        I_new(n - 1) += diff_In * dW(1);
        
        phi_new(0) += diff_phi1 * dW(2);
        phi_new(n - 1) += diff_phin * dW(3);


        // check for blow up (NaN, inf) or negative values
        if (!std::isfinite(I_new(0)) || I_new(0) < 0) {

            #pragma omp critical // prevent garbled output 
            {
                std::cout << "Thread: " << omp_get_thread_num() << std::endl;
                std::cout << "Timestep: " << i << std::endl;
                std::cout << "Time: " << i * dt << std::endl;
                std::cout << "-----------------------------------------" << std::endl;
                std::cout << "Value that failed: I_new(0) = " << I_new(0) << std::endl;
                std::cout << "-----------------------------------------" << std::endl;

                // components that created the bad value
                std::cout << "Value from previous step: I(0) = " << I(0) << std::endl;
                std::cout << "Drift component: " << drift_I(0) * dt << std::endl;
                std::cout << "Stochastic component: " << (diff_I1 * dW(0)) << std::endl;
                std::cout << "Milstein component: " << (milstein_I1 * (dW(0) * dW(0) - dt)) << std::endl;
            }
            // fill energy with NaN to signal an error and stop the thread
