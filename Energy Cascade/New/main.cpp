...
int main() {
    ...
    for (int n : n_values) {
        ...
        outfile.open("energy_nn" + std::to_string(n) + "_no_summation.csv");
        outfile << "mode,energy\n";
        for (int i = 0; i < n; ++i) {
...
// Remove summation terms (2*total_mass_I*I_j) from damping equations
void run_simulation(int n, std::mt19937& rng, Vector& accumulated_energy) {
    ...
    for (int i = 0; i < N_steps; ++i) {
        ...
        // New: 2.0 * T1 - (2.0 * I(0) * I(1) + pow(I(0), 2) + 2.0 * I_next(0) * I(0) * cos(d_phi_next(0)))
        drift_I(0) += 2.0 * gamma * (2.0 * T1 - (2.0 * I(0) * I_next(0) + pow(I(0), 2) + 2.0 * I_next(0) * I(0) * cos(d_phi_next(0))));
        drift_phi(0) += gamma * (2.0 * I_next(0) * sin(d_phi_next(0)));
        
        // same logic to the last mode (n-1)
        drift_I(n - 1) += 2.0 * gamma * (2.0 * Tn - (2.0 * I(n - 1) * I_prev(n - 1) + pow(I(n - 1), 2) + 2.0 * I_prev(n - 1) * I(n - 1) * cos(d_phi_prev(n - 1))));
