...
int main() {
    ...
    for (int n : n_values) {
        ...
        outfile.open("energy_C61n" + std::to_string(n) + ".csv");
        outfile << "mode,energy\n";
        for (int i = 0; i < n; ++i) {
...
void run_simulation(int n, std::mt19937& rng, Vector& accumulated_energy) {
    ...
    for (int i = 0; i < N_steps; ++i) {
        ...
        // Case 5: Same as Case2, remove I_next terms
        drift_I(0) += 2.0 * gamma * (2.0 * T1 - (pow(I(0), 2)));
        // drift_phi(0) += gamma * (2.0 * I_next(0) * sin(d_phi_next(0)));
        
        // drift_I(n - 1) += 2.0 * gamma * (2.0 * Tn - (pow(I(n - 1), 2)));
        ...
        double diff_phi1 = sqrt(2.0 * gamma * T1 );
        double diff_phin = sqrt(2.0 * gamma * Tn );
