...
//
// Bugs in original:
//  (1) Boundary phi drift was using `-=` for Case 0, 2, 3, 4, 6
//      but the SDE in summary eq (3.4) demands `+=` after rewriting
//      sin 2(phi_{j-1}-phi_j) in the code's d_phi_prev/d_phi_next
//      convention. Fixed: all `drift_phi(0) -=` and
//      `drift_phi(n-1) -=` flipped to `+=`.
//  (2) Boundary noise was missing a sqrt(2) factor.
//      Summary eq (3.4) has  noise_I = 2*sqrt(2*gamma*T*I) dW
//                            noise_phi = sqrt(2*gamma*T) * I^{-1/2} dW
//      Old code had `2*sqrt(gamma*T*I)` and `sqrt(gamma*T/I)`,
//      both missing a sqrt(2). Fixed.
// ============================================================
...
void run_simulation(int n, int case_num, double T1, double Tn, int flux_mode, std::mt19937& rng, SimulationResult& result) {
    ...
    while(current_time < T_final)
    {
        ...
        // ============================================================
        // NOISE  (FIX 2: added sqrt(2) factor inside both sqrt terms)
        // Summary eq (3.4):
        //   I noise   :  2 * sqrt(2*gamma*T*I) * dW
        //   phi noise :      sqrt(2*gamma*T)   * I^{-1/2} * dW
        // ============================================================
