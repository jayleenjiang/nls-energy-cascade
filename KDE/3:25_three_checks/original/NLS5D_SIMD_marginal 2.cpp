...
#pragma omp parallel num_threads(N_thread)
    ...
    // This is the exact functional form, uses ALL bins
    {
        ...
        // === Cumulant fit: add cos^2 and cross terms ===
        // log(PDF) = C - a1 cos(th1) - a3 cos(th3) 
        //            + d1 cos^2(th1) + d3 cos^2(th3) + e cos(th1)cos(th3)
        // From cumulant expansion: a1 = <I1I2>/T, d1 = Var(I1I2)/(2T^2)
        MatrixXd X_cum(N_cos, 6);
        for(int k=0; k<N_cos; k++) {
            double c1 = cos1_fit[k], c3 = cos3_fit[k];
            X_cum(k, 0) = 1.0;
            X_cum(k, 1) = -c1;
            X_cum(k, 2) = -c3;
            X_cum(k, 3) = c1*c1;
        ...
        VectorXd beta_cum = (X_cum.transpose() * X_cum).ldlt().solve(X_cum.transpose() * y_cvec);

        cout << endl;
        cout << "=== Cumulant Fit (6-param) ===" << endl;
        cout << "log(PDF) ~ C - a1 cos(th1) - a3 cos(th3) + d1 cos^2(th1) + d3 cos^2(th3) + e cos(th1)cos(th3)" << endl;
        cout << "a1 = " << beta_cum(1) << endl;
        cout << "a3 = " << beta_cum(2) << endl;
        cout << "d1 = " << beta_cum(3) << endl;
        cout << "d3 = " << beta_cum(4) << endl;
        cout << "e  = " << beta_cum(5) << endl;
        cout << endl;

        cout << "=== Cumulant Comparison ===" << endl;
        cout << "a1 * T  = " << beta_cum(1) * T_eq << endl;
        cout << "<I1*I2> = " << mean_I1I2 << endl;
        cout << "Ratio a1*T / <I1*I2> = " << (beta_cum(1) * T_eq) / mean_I1I2 << endl;
        cout << endl;
        cout << "a3 * T  = " << beta_cum(2) * T_eq << endl;
        cout << "<I2*I3> = " << mean_I2I3 << endl;
        cout << "Ratio a3*T / <I2*I3> = " << (beta_cum(2) * T_eq) / mean_I2I3 << endl;
        cout << endl;

        // Theory predicts d1 = Var(I1I2)/(2T^2)
        // so d1 * 2T^2 = Var(I1I2)
        cout << "d1 * 2T^2 = " << beta_cum(3) * 2 * T_eq * T_eq 
             << " (should ≈ Var(I1I2), need to measure)" << endl;
        cout << "d3 * 2T^2 = " << beta_cum(4) * 2 * T_eq * T_eq << endl;
    }
