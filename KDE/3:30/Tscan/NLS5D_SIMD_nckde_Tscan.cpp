...
// ====================================================================
void run_nckde(float T_run, vector<double>& mc_nckde, vector<double>& mc_plain, long long& total_sl) {
    T_run_global = T_run;

    vector<vector<double>> thr_nckde(N_thread, vector<double>(G*G, 0.0));
    vector<vector<double>> thr_plain(N_thread, vector<double>(G*G, 0.0));
...
#pragma omp parallel num_threads(N_thread)
    {
        ...
        for(long long step=0; step<N_sample; step++) {
            ...
            for(int c=0; c<16; c++) {
                ...
                thr_nckde[rank][i1*G + i3] += w;

                // Plain histogram (w=1)
                thr_plain[rank][i1*G + i3] += 1.0;
    ...
    mc_nckde.assign(G*G, 0.0);
    mc_plain.assign(G*G, 0.0);
    total_sl = 0;
    for(int r=0; r<N_thread; r++) {
        for(int k=0; k<G*G; k++) mc_nckde[k] += thr_nckde[r][k];
        for(int k=0; k<G*G; k++) mc_plain[k] += thr_plain[r][k];
        total_sl += thread_counts[r];
    }
    double s1=0, s2=0;
    for(int k=0;k<G*G;k++){ s1+=mc_nckde[k]; s2+=mc_plain[k]; }
    for(int k=0;k<G*G;k++){ mc_nckde[k]/=(s1*H_box*H_box); mc_plain[k]/=(s2*H_box*H_box); }
}
...
#pragma omp parallel num_threads(N_thread)
    ...
    fout << "T_run,rmse_nckde,rmse_plain" << endl;

    printf("%-10s  %-12s  %-12s  %-8s  %s\n", "T_run", "RMSE_NCKDE", "RMSE_PLAIN", "samples", "");
    printf("----------  ------------  ------------  --------  ----\n");

    double best_rmse_n = 1e30, best_T_n = T_target;
    double best_rmse_p = 1e30, best_T_p = T_target;

    for(int it=0; it<N_T; it++) {
        double T_run = T_lo + (T_hi - T_lo) * it / (N_T - 1);

        vector<double> mc_nckde, mc_plain;
        long long nsl;
        run_nckde((float)T_run, mc_nckde, mc_plain, nsl);

        double rmse_n = 0, rmse_p = 0;
        for(int k=0;k<G*G;k++){
            double dn=mc_nckde[k]-theory[k]; rmse_n+=dn*dn;
            double dp=mc_plain[k]-theory[k]; rmse_p+=dp*dp;
        }
        rmse_n = sqrt(rmse_n/(G*G));
        rmse_p = sqrt(rmse_p/(G*G));

        fout << T_run << "," << rmse_n << "," << rmse_p << endl;

        string note = "";
        if(fabs(T_run - T_target) < (T_hi-T_lo)/(N_T-1)*0.6) note = "<-- T_target";
        if(fabs(T_run - T_star) < (T_hi-T_lo)/(N_T-1)*0.6)   note = "<-- T*";

        printf("%-10.4f  %-12.6f  %-12.6f  %-8lld  %s\n", T_run, rmse_n, rmse_p, nsl, note.c_str());

        if(rmse_n < best_rmse_n) { best_rmse_n = rmse_n; best_T_n = T_run; }
        if(rmse_p < best_rmse_p) { best_rmse_p = rmse_p; best_T_p = T_run; }
    }

    printf("\n>> NC-KDE BEST: T_run=%.4f  RMSE=%.6f\n", best_T_n, best_rmse_n);
    printf(">> PLAIN  BEST: T_run=%.4f  RMSE=%.6f\n", best_T_p, best_rmse_p);
