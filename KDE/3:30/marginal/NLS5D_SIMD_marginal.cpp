...
#pragma omp parallel num_threads(N_thread)
    ...
    // === Cos fitting: log(PDF) = C - a1*cos(theta1) - a3*cos(theta3) ===
    // This is the exact functional form, uses ALL bins
    {
        vector<double> y_cos, cos1_fit, cos3_fit;
        for(int i=0; i<G; i++) {
            double th1 = Th_lo + (i+0.5)*H_box;
            for(int j=0; j<G; j++) {
                double th3 = Th_lo + (j+0.5)*H_box;
                double p = pdf[i*G+j];
                if(p > 0) {
                    y_cos.push_back(log(p));
                    cos1_fit.push_back(cos(th1));
                    cos3_fit.push_back(cos(th3));
                }
            }
        }
        int N_cos = y_cos.size();
        MatrixXd X_cos(N_cos, 3);
        VectorXd y_cvec(N_cos);
        for(int k=0; k<N_cos; k++) {
            X_cos(k, 0) = 1.0;
            X_cos(k, 1) = -cos1_fit[k];
            X_cos(k, 2) = -cos3_fit[k];
            y_cvec(k) = y_cos[k];
        }
        Vector3d beta_cos = (X_cos.transpose() * X_cos).ldlt().solve(X_cos.transpose() * y_cvec);
        double a1_cos = beta_cos(1);
        double a3_cos = beta_cos(2);

        cout << endl;
        cout << "=== Cos Fit (3-param) ===" << endl;
        cout << "log(PDF) ~ C - a1*cos(theta1) - a3*cos(theta3)" << endl;
        cout << "a1 = " << a1_cos << ", a3 = " << a3_cos << endl;
        cout << "a1*T / <I1I2> = " << (a1_cos * T_eq) / mean_I1I2 << endl;
        cout << "a3*T / <I2I3> = " << (a3_cos * T_eq) / mean_I2I3 << endl;
    }

    // === Quadratic fit near PEAK (theta ≈ ±pi) ===
    // cos(theta) ≈ -1 + (theta-pi)^2/2 near theta=pi
    // log(rho) ≈ C - A*(theta1-pi)^2 - B*(theta3-pi)^2, A > 0
    // Fit using bins with |theta_j - pi| < pi/2 OR |theta_j + pi| < pi/2
    {
        vector<double> y_pk, d1sq_pk, d3sq_pk;
        for(int i=0; i<G; i++) {
            double th1 = Th_lo + (i+0.5)*H_box;
            // distance to nearest ±pi
            double dist1 = min(fabs(th1 - PI_f), fabs(th1 + PI_f));
            for(int j=0; j<G; j++) {
                double th3 = Th_lo + (j+0.5)*H_box;
                double dist3 = min(fabs(th3 - PI_f), fabs(th3 + PI_f));
                double p = pdf[i*G+j];
                if(p > 0 && dist1 < PI_f/2 && dist3 < PI_f/2) {
                    y_pk.push_back(log(p));
                    d1sq_pk.push_back(dist1*dist1);
                    d3sq_pk.push_back(dist3*dist3);
                }
            }
        }
        int N_pk = y_pk.size();
        cout << endl;
        cout << "=== Quadratic Fit near PEAK (theta ≈ ±pi) ===" << endl;
        cout << "Fitting " << N_pk << " bins near corners" << endl;

        MatrixXd X_pk(N_pk, 3);
        VectorXd y_pvec(N_pk);
        for(int k=0; k<N_pk; k++) {
            X_pk(k, 0) = 1.0;
            X_pk(k, 1) = -d1sq_pk[k];
            X_pk(k, 2) = -d3sq_pk[k];
            y_pvec(k) = y_pk[k];
        }
        Vector3d beta_pk = (X_pk.transpose() * X_pk).ldlt().solve(X_pk.transpose() * y_pvec);
        double A_pk = beta_pk(1);
        double B_pk = beta_pk(2);

        cout << "log(PDF) ~ C - A*(theta1-pi)^2 - B*(theta3-pi)^2" << endl;
        cout << "A = " << A_pk << "  (expect > 0, density maximum at corners)" << endl;
        cout << "B = " << B_pk << endl;
        cout << endl;
        cout << "A * 2T  = " << A_pk * 2 * T_eq << endl;
        cout << "<I1*I2> = " << mean_I1I2 << endl;
        cout << "Ratio A*2T / <I1*I2> = " << (A_pk * 2 * T_eq) / mean_I1I2 << endl;
        cout << endl;
        cout << "B * 2T  = " << B_pk * 2 * T_eq << endl;
        cout << "<I2*I3> = " << mean_I2I3 << endl;
        cout << "Ratio B*2T / <I2*I3> = " << (B_pk * 2 * T_eq) / mean_I2I3 << endl;
    }
