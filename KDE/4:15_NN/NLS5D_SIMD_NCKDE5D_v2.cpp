/*
 * NLS5D_SIMD_NCKDE5D_v2.cpp
 *
 * 5D steady-state density estimation with Noise-Compensating KDE.
 * 
 * v2 changes: T1=T3=5 (equilibrium), I_max=10, N_I=50, N_Th=30, h=0.06
 *
 * Compile (Mac):
 *   clang++ -O3 -std=c++17 -I/opt/homebrew/include/eigen3 \
 *       -Xpreprocessor -fopenmp -I/opt/homebrew/opt/libomp/include \
 *       -L/opt/homebrew/opt/libomp/lib -lomp NLS5D_SIMD_NCKDE5D_v2.cpp -o nls5d_nckde2
 *
 * Usage:
 *   ./nls5d_nckde2 [N_thread] [gamma] [T1] [T3] [N_box] [N_sample]
 *   e.g.: ./nls5d_nckde2 8 0.1 5.0 5.0 50000 100000000
 */
...
const int N_I  = 50;
const int N_Th = 30;
const float hI  = (I_max - I_lo) / N_I;   // = 0.2
const float hTh = (Th_hi - Th_lo) / N_Th; // = 2*pi/30 ~ 0.2094
...
// NC-KDE bandwidth (advisor specified h = 0.06, used for all dimensions)
const float sigma_I  = 0.06f;
const float sigma_Th = 0.06f;
