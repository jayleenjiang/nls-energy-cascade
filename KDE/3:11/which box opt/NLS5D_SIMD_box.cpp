...
// ================
// Hash map for O(1) box lookup
// ================
// Key = flatten 5D grid index: (n1, n2, n3, nt1, nt3) -> unique uint64
unordered_map<uint64_t, int> box_map;

inline uint64_t grid_key(int n1, int n2, int n3, int nt1, int nt3) {
    // N_I=150, N_Th=150, so each index fits in 8 bits
    return ((uint64_t)n1 << 32) | ((uint64_t)n2 << 24) | ((uint64_t)n3 << 16) 
         | ((uint64_t)nt1 << 8) | (uint64_t)nt3;
}
...
// =============
// Box lookup (hash map, O(1))
// ==============
inline int which_box(const Coords5D& c) {
    ...
    auto it = box_map.find(grid_key(n1, n2, n3, nt1, nt3));
    return (it != box_map.end()) ? it->second : -1;
}

// ===============
// Register box (hash map)
// ==============
bool register_box(const Coords5D& c, int box_id) {
    ...
    uint64_t key = grid_key(n1, n2, n3, nt1, nt3);
    if(box_map.count(key)) return false;  // already occupied
    box_map[key] = box_id;
    return true;
}
...
void create_boxes(MatrixXf& Boxes) {
...
// ==========
// Load boxes from file (for cross-version comparison)
// ==========
void load_boxes(const string& filename, MatrixXf& Boxes) {
    ifstream fin(filename);
    if(!fin.is_open()) { cerr << "Cannot open " << filename << endl; exit(1); }
    
    int count = 0;
    float b0, b1, b2, b3, b4;
    while(fin >> b0 >> b1 >> b2 >> b3 >> b4 && count < N_box) {
        Boxes(0, count) = b0;
        Boxes(1, count) = b1;
        Boxes(2, count) = b2;
        Boxes(3, count) = b3;
        ...
        // Reconstruct grid indices from box centers
        int n1  = max(0, min((int)floorf((b0 - hI/2.0f - I_lo) / hI + 0.5f), N_I-1));
        int n2  = max(0, min((int)floorf((b1 - hI/2.0f - I_lo) / hI + 0.5f), N_I-1));
        int n3  = max(0, min((int)floorf((b2 - hI/2.0f - I_lo) / hI + 0.5f), N_I-1));
        int nt1 = max(0, min((int)floorf((b3 - hTh/2.0f - Th_lo) / hTh + 0.5f), N_Th-1));
        int nt3 = max(0, min((int)floorf((b4 - hTh/2.0f - Th_lo) / hTh + 0.5f), N_Th-1));
        
        box_map[grid_key(n1, n2, n3, nt1, nt3)] = count;
        count++;
    }
    fin.close();
    N_box = count;
    cout << "Loaded " << count << " boxes from " << filename << endl;
}
...
int main(int argc, char* argv[]) {
    ...
    string load_boxes_file = "";
    // Parse args: check for --load-boxes first
    int argi = 1;
    if(argi < argc && string(argv[argi]) == "--load-boxes") {
        argi++;
        if(argi < argc) load_boxes_file = argv[argi++];
    }
    if(argi < argc) N_thread  = atoi(argv[argi++]);
    if(argi < argc) gamma_val = atof(argv[argi++]);
    if(argi < argc) T1        = atof(argv[argi++]);
    if(argi < argc) T3        = atof(argv[argi++]);
    if(argi < argc) N_box     = atoi(argv[argi++]);
    if(argi < argc) N_sample  = atoll(argv[argi++]);
    ...
    if(!load_boxes_file.empty()) {
        load_boxes(load_boxes_file, Boxes);
    } else {
        cout << "Creating boxes..." << endl;
        create_boxes(Boxes);
    }
    cout << "box_map size = " << box_map.size() << endl;
