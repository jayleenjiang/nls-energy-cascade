#!/usr/bin/env python3
"""
Flux distribution + exponential-tail analysis for the NLS energy-cascade chain.
Reads flux_n<n>.txt (one time-averaged-flux sample per line, '#' header) from
NLS_flux_SIMD.cpp, for n in N_LIST, and:
  (1) plots the flux PDFs,
  (2) plots the upper-tail survival P[flux > x] on a log axis,
  (3) fits the exponential tail log P[flux > x] = -lambda x + c over a SURVIVAL
      WINDOW [S_LO, S_HI] that excludes both the near-Gaussian core and the far
      faster-than-exponential cutoff,
and prints a summary table (mean, std, skew, lambda, R^2, fit window in x).

Usage:  python3 flux_tail_analysis.py
Tune S_LO / S_HI below.
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
N_LIST = [10, 20, 30, 40]
# Fit the exponential tail over a SURVIVAL-PROBABILITY window [S_LO, S_HI]:
