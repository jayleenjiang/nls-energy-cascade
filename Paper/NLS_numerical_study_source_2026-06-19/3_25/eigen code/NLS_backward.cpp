 ...
 * NLS_backward.cpp
 *
 * Backward solver for generator eigenfunctions of 3-mode stochastic NLS.
 * Adapted from Kuramoto_SIMD_backward.cpp.
 *
 ...
 *       -L/opt/homebrew/opt/libomp/lib -lomp NLS_backward.cpp -o nls_backward
 *
 * Usage:
 *   ./nls_backward [N_thread] [gamma] [T1] [T3] [N_sample]
 *   e.g.: ./nls_backward 8 0.1 5.0 5.0 1000    (quick test)
 *         ./nls_backward 8 0.1 5.0 5.0 10000   (production)
 */
...
// ====================================================================
// Physical parameters (can be overridden by command line)
// ====================================================================
...
// ====================================================================
int N_initials      = 64000;    // number of initial points (must be multiple of 16)
int T_steps         = 500;      // number of recorded time steps
int gap             = 10;     // record every gap*dt time units (gap*dt = 1.0)
long long N_sample  = 10000;    // MC trajectories per initial point
float obs_cutoff    = 3.0f;     // cutoff for localized observable
int N_thread        = 8;

// ====================================================================
// SIMD types and fast math (from NLS5D_SIMD_NCKDE5D_v2.cpp)
// ====================================================================
...
// ====================================================================
// NLS Euler-Maruyama step (from NLS5D_SIMD_NCKDE5D_v2.cpp, verified)
// ====================================================================
...
// ====================================================================
// Observable: cos(theta1) = cos(2*(phi1 - phi2))
// Sensitive to angle dynamics / phase locking oscillation
// ====================================================================
inline float observable_scalar(const State6x16& X, int col) {
    float p1 = X(3, col), p2 = X(4, col);
...
#pragma omp for schedule(dynamic)
        for (long long int n = 0; n < N_initials / 16; n++) {
            ...
            for (int m = 0; m < N_sample; m++) {
                ...
                for (int j = 0; j <= T_steps * gap; j++) {
                    ...
                    if (j % gap == 0) {
                        int t_idx = j / gap;
                        for (int i = 0; i < 16; i++) {
                            Obs_traj(n * 16 + i, t_idx) += observable_scalar(state_batch, i);
                        }
                    }
                    // EM step
                    if (j < T_steps * gap) {
            ...
            // Progress (only thread 0 prints)
            if (rank == 0 && n % 50 == 0) {
                cout << "  batch " << n*16 << "/" << N_initials << " (" 
                     << (100*n*16/N_initials) << "%)" << endl;
            }
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    cout << "Observable: cos(theta1) = cos(2*(phi1-phi2))" << endl;
