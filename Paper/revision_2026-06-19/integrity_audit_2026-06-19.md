...
| Data/code and figure path availability | data/code availability statement and all manuscript figure includes | `availability_path_audit.json`; `availability_path_audit.md`; script `scripts/audit_availability_paths.py` | VERIFIED locally for 34/34 referenced paths after adding the LTE mesh and finite-window current figures. This check proves local path existence and hashes, but should be rerun against the final release branch/archive. |
...
3. Rerun the final citation/data/claim/path audit after author declarations and
   ...
   passes 34/34 checks; the one-command runner
