...
| Core numerical manuscript claims | abstract, long-chain section, LTE table, LTE residual decomposition, short-chain section, reproducibility summary, data availability | `manuscript_claim_audit.json`; `manuscript_claim_audit.md`; script `scripts/audit_manuscript_claims.py` | VERIFIED for 20/20 code-verifiable numerical/data claims after the larger-chain, bath-temperature, thermostat-coupling, and LTE residual-decomposition updates. This audit also caught and corrected the LTE notation so the local equilibrium marginal uses the manuscript convention `exp[-H/(2T)]`, not the ambiguous `exp(-H/beta)`. |
...
| Compiled-PDF layout QA | generic `article` PDF layout and updated figure-page rendering checks | `pdf_layout_qa_2026-06-19.md` | PASS WITH LIMITATIONS: the 24-page A4 PDF has no scanned LaTeX-log warnings, and the newly added LTE mesh/current-diagnostic pages render without visible clipping after white-background conversion. A target-journal proof pass is still required after template conversion. |
...
3. Rerun the final citation/data/claim/path audit after author declarations and
   ...
   numerical claim audit passes 20/20 checks and the availability-path audit
