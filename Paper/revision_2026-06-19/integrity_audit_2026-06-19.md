...
| Compiled-PDF layout QA | rendered generic `article` PDF layout and text-extraction sanity checks | `pdf_layout_qa_2026-06-19.md` | PASS WITH LIMITATIONS: the 18-page A4 PDF has no scanned warnings, unresolved-reference markers, placeholder tokens, blank pages, or obvious visible clipping in the rendered-page inspection. A target-journal proof pass is still required after template conversion. |
