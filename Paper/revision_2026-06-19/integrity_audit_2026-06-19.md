...
| Core numerical manuscript claims | abstract, long-chain section, LTE table, short-chain section, reproducibility summary, data availability | `manuscript_claim_audit.json`; `manuscript_claim_audit.md`; script `scripts/audit_manuscript_claims.py` | VERIFIED for 18/18 code-verifiable numerical/data claims after the larger-chain and bath-temperature robustness updates. This audit also caught and corrected the LTE notation so the local equilibrium marginal uses the manuscript convention `exp[-H/(2T)]`, not the ambiguous `exp(-H/beta)`. |
...
3. Rerun the final citation/data/claim/path audit after author declarations and
   ...
   numerical claim audit passes 18/18 checks and the availability-path audit
