...
- Current code-verifiable numerical claim audit: 14/14 claims verified.
- Current availability-path audit: 34/34 paths present and required files
  git-tracked.
...
| S3 | `experiments/flux_validation/` | Current-scaling validation artifacts, production summaries, current-window diagnostics, and validation report. | Supports the main action-current scaling and finite-window current diagnostics. |
...
| S5 | `raw_data_archive_manifest.md` plus DOI-backed raw-data archive, if created | Minimal source-traced raw-data subset: 40 files, 138,875,181 bytes, paths preserved under `raw_data/`. | Needed only if SIADS/referees require raw histograms/model files beyond the tracked derived artifacts. |
