...
| Core numerical manuscript claims | abstract, long-chain section, LTE table, short-chain section, data availability | `manuscript_claim_audit.json`; `manuscript_claim_audit.md`; script `scripts/audit_manuscript_claims.py` | VERIFIED for 13/13 code-verifiable numerical/data claims. This audit also caught and corrected the LTE notation so the local equilibrium marginal uses the manuscript convention `exp[-H/(2T)]`, not the ambiguous `exp(-H/beta)`. |
...
3. Rerun the final citation/data/claim audit after author declarations and any
   ...
   claim audit passes 13/13 checks, but it should be repeated after final edits.
