#!/usr/bin/env python3
"""Transport density ratio with a frozen two-moment exponential correction."""
...
class DoubleCalibratedTransportGibbsRatio(TransportGibbsRatio):
    def __init__(
    ...
    def calibration_observables(self, state: tf.Tensor) -> tf.Tensor:
        ...
        i1, i2, _, th1, _ = tf.unstack(state, axis=-1)
        return tf.stack((i1, i1 * i2 * tf.sin(th1)), axis=-1)
    ...
    def save_bundle(self, directory: Path, extra: dict) -> None:
        ...
        config = {
            "model_type": "transport_hybrid_double_moment_calibration",
...
def load_bundle(directory: Path) -> DoubleCalibratedTransportGibbsRatio:
    directory = Path(directory)
    config = json.loads((directory / "model_config.json").read_text())
    model = DoubleCalibratedTransportGibbsRatio(
