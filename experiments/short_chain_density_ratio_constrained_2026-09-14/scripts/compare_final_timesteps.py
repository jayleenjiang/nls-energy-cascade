#!/usr/bin/env python3
"""Compare final fine and coarse density ratios on both sampled supports."""
...
def main():
    ...
    fine = [load_fine_bundle(path) for path in args.fine_model]
    coarse = [load_coarse_bundle(path) for path in args.coarse_model]
    rows = []; ensemble = {}
    for support in ("fine", "coarse"):
        ...
        state = state[indices(len(state), 50_000, f"final-timestep|{support}")]
        fv = np.asarray([values(model, state) for model in fine])
        cv = np.asarray([values(model, state) for model in coarse])
        fv -= fv.mean(axis=1, keepdims=True); cv -= cv.mean(axis=1, keepdims=True)
        for index in range(3):
            rms = float(np.sqrt(np.mean((fv[index] - cv[index]) ** 2)))
            rows.append({
                "support": support, "comparison": f"paired_seed_{index + 1}",
        ...
        rows.append({
            "support": support, "comparison": "ensemble_mean",
            "centered_log_ratio_rms": rms, "gate": 0.10,
            "pass": int(rms <= 0.10),
        })
    ...
    result = {
        ...
        "maximum_ensemble_rms": max(ensemble.values()), "gate": 0.10,
