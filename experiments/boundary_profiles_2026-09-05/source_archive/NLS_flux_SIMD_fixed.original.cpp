...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <case_number> <T1> <Tn> <n> [batches] [T_final] [seed]" << std::endl;
    ...
    if (argc >= 8) base_seed = static_cast<uint32_t>(std::strtoul(argv[7], nullptr, 10));
    float deltaT = T1 - Tn;
...
*/
    ...
    double std_error = std_dev / sqrt(static_cast<double>(total_trajectories));
    ...
    std::cout << "Std Error = " << std_error << std::endl;
    ...
    std::cout << "Results appended to flux_vs_length_2.csv" << std::endl;
