#!/usr/bin/env python3
"""Gibbs-anchored smooth periodic density-ratio model."""
...
class GibbsRatioDensity(tf.keras.Model):
    def __init__(self, architecture: str, action_scales: np.ndarray, seed: int):
        super().__init__(name="gibbs_ratio_density")
        ...
        self.hidden = [
            tf.keras.layers.Dense(
                ...
                bias_initializer="zeros", name=f"hidden_{j}",
            ) for j, width in enumerate(self.widths)
        ]
        self.output_layer = tf.keras.layers.Dense(
            1, dtype=tf.float64, kernel_initializer="zeros", bias_initializer="zeros",
            name="log_ratio",
        )
    ...
    def features(self, state: tf.Tensor) -> tf.Tensor:
        ...
        return tf.stack((
            ...
            tf.sin(th1), tf.cos(th1), tf.sin(th3), tf.cos(th3),
            tf.sin(th1-th3), tf.cos(th1-th3),
        ), axis=-1)
    ...
    def log_prob(self, state: tf.Tensor) -> tf.Tensor:
        # The additive Gibbs partition constant and density-ratio normalizer do
        # not affect derivatives. Evaluation records the latter separately.
...
def load_bundle(directory: Path) -> GibbsRatioDensity:
    directory = Path(directory)
    config = json.loads((directory/"model_config.json").read_text())
    model = GibbsRatioDensity(
