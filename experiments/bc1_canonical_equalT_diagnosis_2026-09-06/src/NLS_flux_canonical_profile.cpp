...
//
// This diagnostic variant measures the finite-time averaged ACTION current,
// the complete mean-action profile, and the complete bond-sine profile
//
...
struct BatchResult {
    ...
    AlignedVec sin_integral;
    std::array<A16d, N_CURRENT_BLOCKS> current_integral_blocks{};
    std::uint64_t projection_count = 0;
    std::uint64_t near_floor_count = 0;
    double minimum_proposed_action = std::numeric_limits<double>::infinity();
...
MeanSE mean_se(const std::vector<double>& values) {
    MeanSE result;
    double sum = 0.0;
...
BatchResult run_batch(const Parameters& p, int batch_index) {
    ...
    result.sin_integral.resize(p.n - 1);
    for (auto& value : result.sin_integral) {
        value.setZero();
    }
    ...
    for (std::int64_t step = 0; step < total_steps; ++step) {
        ...
        if (step >= p.burn_steps) {
            ...
            for (int j = 1; j < p.n; ++j) {
                result.sin_integral[j - 1] += sin_bond[j] * p.dt;
            }
        }
        ...
        for (int j = 0; j < p.n; ++j) {
            result.minimum_proposed_action = std::min(
                result.minimum_proposed_action, action[j].minCoeff());
            result.near_floor_count += static_cast<std::uint64_t>(
                (action[j] <= 10.0 * ACTION_FLOOR).count());
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::vector<std::vector<double>> trajectory_action(
            p.n, std::vector<double>(trajectory_count, 0.0));
        std::vector<std::vector<double>> trajectory_sin(
            p.n - 1, std::vector<double>(trajectory_count, 0.0));
        ...
        std::vector<std::uint64_t> batch_near_floor(p.batches, 0);
        std::vector<double> batch_minimum_action(
            p.batches, std::numeric_limits<double>::infinity());
        ...
        for (int batch = 0; batch < p.batches; ++batch) {
            ...
            for (int lane = 0; lane < LANES; ++lane) {
                ...
                for (int j = 0; j < p.n; ++j) {
                    trajectory_action[j][trajectory] =
                        result.action_integral[j](lane) / p.measure;
                }
                for (int j = 0; j < p.n - 1; ++j) {
                    trajectory_sin[j][trajectory] =
                        result.sin_integral[j](lane) / p.measure;
                }
            }
            ...
            batch_near_floor[batch] = result.near_floor_count;
            batch_minimum_action[batch] = result.minimum_proposed_action;
        ...
        std::vector<MeanSE> action_statistics(p.n);
        std::vector<MeanSE> sine_statistics(p.n - 1);
        for (int j = 0; j < p.n; ++j) {
            action_statistics[j] = mean_se(trajectory_action[j]);
        }
        for (int j = 0; j < p.n - 1; ++j) {
            sine_statistics[j] = mean_se(trajectory_sin[j]);
        }
        ...
        const std::uint64_t projection_count =
            std::accumulate(batch_projections.begin(),
                            ...
                            std::uint64_t{0});
        const std::uint64_t near_floor_count =
            std::accumulate(batch_near_floor.begin(),
                            batch_near_floor.end(),
                            std::uint64_t{0});
        const double minimum_proposed_action = *std::min_element(
            batch_minimum_action.begin(), batch_minimum_action.end());
        ...
        }
        {
            ...
            output
                << "j,mean_I,se_mean_I,mean_sin_theta,"
                << "se_mean_sin_theta,sample_count\n";
            for (int j = 0; j < p.n; ++j) {
                output << j + 1 << ","
                       << action_statistics[j].mean << ","
                       << action_statistics[j].se << ",";
                if (j < p.n - 1) {
                    output << sine_statistics[j].mean << ","
                           << sine_statistics[j].se;
                } else {
                    output << ",";
                }
                output << "," << action_statistics[j].count << "\n";
            }
        }
        {
            auto output = open_output(p.prefix + "_trajectory_profiles.csv");
            output << "trajectory_id,j,mean_I,mean_sin_theta\n";
            for (int trajectory = 0; trajectory < trajectory_count; ++trajectory) {
                for (int j = 0; j < p.n; ++j) {
                    output << trajectory << "," << j + 1 << ","
                           << trajectory_action[j][trajectory] << ",";
                    if (j < p.n - 1) {
                        output << trajectory_sin[j][trajectory];
                    }
                    output << "\n";
                }
            }
        ...
        }
        {
            ...
            output
                ...
                << "action_floor,projection_count,near_floor_count,"
                << "minimum_proposed_action,"
            ...
            output
                ...
                << near_floor_count << "," << minimum_proposed_action << ","
                << projection_rate << ","
        ...
        std::cout << std::setprecision(10)
                  ...
                  << ", near-floor count = " << near_floor_count
                  << ", minimum proposed action = " << minimum_proposed_action
